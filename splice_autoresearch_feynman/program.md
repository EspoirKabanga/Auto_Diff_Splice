...
## Mission
1. Modify `splice_diffusion_feynman.py` to implement the Feynman-Kac Corrector algorithm (e.g. SDE weighting and tilting marginals based on the prior likelihoods). You can refer to DISCO paper's continuous or discrete FKC derivations.
...
1. Read `splice_diffusion_feynman.py`. Understand how it currently loops in reverse diffusion (reverse timestep loop `for i in reversed(range(timesteps)):`).
2. Read `evaluate_sequences.py` to understand evaluating logic. **Do not modify this file.**
3. Create experiments by implementing logic directly into `splice_diffusion_feynman.py`. Use the dictionaries `full_prev_dist` and `full_next_dist` provided as args for estimating your positional reward $h_t(x)$ in the FKC math during sampling.

## Execution and Logging
1. Modify `splice_diffusion_feynman.py` locally.
